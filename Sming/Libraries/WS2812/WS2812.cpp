...
void ICACHE_FLASH_ATTR ws2812_writebyte(uint8_t gpio, char data) {
	uint8_t mask = 0x80;
	while (mask) {
		(data & mask) ? send_ws_1(gpio) : send_ws_0(gpio);
		mask >>= 1;
	}
}

// Byte triples in the buffer as R G B and send to hardware as G R B
int ICACHE_FLASH_ATTR ws2812_writergb(uint8_t gpio, char *buffer, size_t length)
{
	...
	// Should not rearrange buffer. It is a bad practice to modify input data.
	/*size_t i;
	...
	while (buffer != end) {
		ws2812_writebyte(gpio, *(buffer + 1));//green
		ws2812_writebyte(gpio, *(buffer));//red
		ws2812_writebyte(gpio, *(buffer + 2));//blue
		buffer += 3;
	}
	ets_intr_unlock();
}